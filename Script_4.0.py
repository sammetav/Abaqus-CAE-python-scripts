...
b = Base(2.7e-09, 70000, 0.33, 15, 'z', 0.06, 'displacement')
magnitude = [-15, -15, -42]
#magnitude = [-150, -150, -400]
...
##########################################################################
# commenting the code
# improve the automization in the code 
# Tests FD curves (other geometries)
# As parametric as possible
# ppt 
...
# INTERACTION CREATION FOR SELF CONTACT
...
#IF CONTACT PLATE IS TRUE, CONTACT PLATE GEOMETRY CREATION; MATERIAL ASSIGNMENT; MESH OF IT
if Contactplate:
...
#Contact plate orientation in assembly
        a = mdb.models['Model-1'].rootAssembly
...
#surface creation of the contact plate 
...
#constraint the plate to the RP
...
#surfaces of the plates
        ...
        faces1 = f1.getSequenceFromMask(mask=('[#1 ]', ), )
        a.Set(faces=faces1, name='X-Plate-Upper')
#constraints of the plate to RP
        mdb.models['Model-1'].Equation(name='Constraint-x1', terms=((-1.0, 
            'X-Plate-Lower', 1), (0.5, 'RP4', 1)))
        mdb.models['Model-1'].Equation(name='Constraint-x2', terms=((1.0, 
            'X-Plate-Upper', 1), (0.5, 'RP4', 1)))
...
if b.direction == 'Multi' and b.BoundaryCondition == 'displacement':
    displacement='U3'
    Reactionforce='RF3'
#BC1, BC2, BC3 in all three directions
    a=mdb.models['Model-1'].rootAssembly
    region=a.sets['RP4']
...
#Explicit General contact exclude surfaces in the contact
